...
docker tag item-app:v1 rafiannas/item-app:v1     #menjelaskan perubahan nama image agar sesuai dengan format docker hub

echo $PASSWORD_DOCKER_HUB | docker login -u rafiannas --password-stdin      #login ke docker hub

docker push rafiannas/item-app:v1         #menjelaskan pengunggahan image ke docker hub